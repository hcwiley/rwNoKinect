...
class EImage {
  ...
  EImage(rwNoKinect sce, String file) {
    parent = sce;
    position = new int[3];
  ...
  void render(int offset, int total, int threshHold, int blockSpacing) {
    ...
    // extrude.width);
    parent.beginShape(rwNoKinect.QUADS);
    for (int x = forDelta; x < extrude.width; x += forDelta) {
      for (int y = forDelta; y < extrude.height; y += forDelta) {
        int r = parent.buildingRadius;
        if (values[(x * extrude.width) + y] < threshHold) {
          int inverted = (int) (rwNoKinect.map(
          (int) values[(x * extrude.width) + y], 0, 
          255, 255, 0));
          r += values[(x * extrude.width) + y] * forDelta;
          int ymult = (int) rwNoKinect.map(parent.buildingRadius, 400, 
          3000, 4, 18);
          position[0] = (int) (r * rwNoKinect.cos(theta));
          position[1] = y * ymult - parent.buildingRadius
            - extrude.height;
          position[2] = (int) (r * rwNoKinect.sin(theta));
...
abstract class Cube {
  ...
  rwNoKinect parent, int blockSpacing) {
    //        parent.fill(color);
    parent.stroke(255);
    // face 1
    parent.vertex(x, y, z);
